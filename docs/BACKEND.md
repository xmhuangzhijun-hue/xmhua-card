...
Self-service creation fails closed when the database, feature flag, or invite code is absent. The route also enforces bounded request/field sizes, separate in-process limits for invalid invitations and valid creation attempts, and `MAX_TENANTS`. Invalid invite codes do not consume the valid site-creation bucket, while repeated invalid guesses are independently throttled. The in-memory key store is bounded, and self-service tenant counting plus insertion is serialized with a PostgreSQL transaction-scoped advisory lock. Multi-instance public deployments still need shared gateway-level rate limiting for distributed abuse. Set `TRUST_PROXY_HEADERS=true` only behind a trusted proxy that overwrites forwarded headers.
