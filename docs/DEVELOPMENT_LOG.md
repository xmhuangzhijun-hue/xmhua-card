...
## 2026-09-12 — WeChat Local Toolkit portfolio entry

Added the author's sanitized open-source toolkit through the content API and reviewed its repository for hourly GitHub facts. Existing project prose and relative order are preserved. Exactly one API mapping line is deployed, with candidate/rollback/final startup checks and first snapshot readback. See reports/2026-09-12-wechat-project.md for scope, regressions, and the existing read-only ISR cache limitation. No frontend permissions or dependencies changed.
