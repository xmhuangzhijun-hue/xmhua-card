...
export function HooosbergClone() {
  ...
  return <>
    ...
    <main>
      ...
      <section className="landing-section home-products-section" id="products"><Heading {...sections.products}/><div className="product-grid product-grid--catalog product-grid--home">{products.map(product=><article className="product-card" key={product.id}><div className="product-card__top"><span className="product-icon product-icon--md"><img src={product.image} alt=""/></span><div><h3>{product.name}</h3><p>{product.subtitle}</p></div></div><p className="product-card__summary">{product.summary}</p><div className="product-card__meta"><span>{product.platform}</span><span>{ui.productStoreLabel}</span></div><div className="product-card__actions"><a className="text-action product-card__button" href={product.href}>{ui.productStoreLabel}</a><a className="text-action product-card__button" href={product.href}>{ui.productNotesLabel} <ArrowRight size={15}/></a></div></article>)}</div></section>
    ...
    <footer className="site-footer"><div className="site-footer__inner"><div className="site-footer__brand"><strong>{site.brandName}</strong><p>{footer.description}</p></div><nav className="site-footer__social">{socials.map(social=><a href={social.href} key={social.id}>{social.label}</a>)}<a href={ui.emailLink.href}>{ui.emailLink.label}</a></nav><nav className="site-footer__legal">{footer.legalLinks.map(link=><a href={link.href} key={link.label}>{link.label}</a>)}</nav><p className="site-footer__note">{footer.note} <span>{footer.copyright}</span></p></div></footer>
    {consent&&ui.analytics.enabled&&<div className="analytics-consent"><div className="analytics-consent__copy"><strong>{ui.analytics.title}</strong><p>{ui.analytics.description}</p><div className="analytics-consent__links"><a href={ui.analytics.privacyLink.href}>{ui.analytics.privacyLink.label}</a><a href={ui.analytics.cookieLink.href}>{ui.analytics.cookieLink.label}</a></div></div><div className="analytics-consent__actions"><button className="button button--secondary" onClick={()=>setConsent(false)}>{ui.analytics.rejectLabel}</button><button className="button button--primary" onClick={()=>setConsent(false)}>{ui.analytics.acceptLabel}</button></div></div>}
  </>;
}
