  ...
  return <main className="admin-shell">
    ...
    <div className="admin-grid">
      <aside className="tenant-rail">
        ...
        <div className="key-panel"><label htmlFor="admin-key"><KeyRound size={15}/> 远程管理密钥（本机可留空）</label><input id="admin-key" type="password" autoComplete="off" placeholder="本机自动授权" value={key} onChange={event => setKey(event.target.value)}/><button onClick={() => connect()} disabled={state === "loading"}>{state === "loading" ? <LoaderCircle className="spin" size={15}/> : <Database size={15}/>}连接数据库</button></div>
      </aside>
