...
export function ContentAdmin() {
  ...
  // Connect once on mount; later reconnects are explicit user actions.
  // eslint-disable-next-line react-hooks/exhaustive-deps
  useEffect(() => { const stored = sessionStorage.getItem("xmhua-admin-key") ?? ""; setKey(stored); const timeout = window.setTimeout(() => { void connect("xmhua", stored); }, 0); return () => window.clearTimeout(timeout); }, []);
  ...
  return <main className="admin-shell">
    ...
    <div className="admin-grid">
      <aside className="tenant-rail">
        ...
        <div className="key-panel"><label htmlFor="admin-key"><KeyRound size={15}/> 平台管理密钥</label><input id="admin-key" type="password" autoComplete="off" placeholder="输入 ADMIN_API_KEY" value={key} onChange={event => setKey(event.target.value)}/><button onClick={() => connect()} disabled={state === "loading"}>{state === "loading" ? <LoaderCircle className="spin" size={15}/> : <Database size={15}/>}连接数据库</button></div>
      </aside>
      <section className="editor-panel">
        <div className="editor-toolbar"><div><p>CONTENT DOCUMENT</p><h2>{tenants.find(item => item.slug === tenant)?.name ?? tenant}</h2></div><div className="editor-actions"><button onClick={() => connect()} disabled={state === "loading"}><RefreshCw size={15}/>重新读取</button><button className="primary" onClick={save} disabled={!valid || state === "saving"}>{state === "saving" ? <LoaderCircle className="spin" size={15}/> : <Save size={15}/>}保存到数据库</button></div></div>
