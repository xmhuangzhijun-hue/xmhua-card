    ...
    {content && <div className="studio-layout">
      <section className="studio-form">
        ...
        <EditorSection title="文章">
          {content.articles.map((article, index) => <div className="repeat-card" key={article.id}><button className="remove" aria-label="删除文章" onClick={() => setContent({ ...content, articles: content.articles.filter((_, itemIndex) => itemIndex !== index) })}><Trash2 /></button><Field label="标题" value={article.title} onChange={value => setContent({ ...content, articles: content.articles.map((item, itemIndex) => itemIndex === index ? { ...item, title: value } : item) })} /><Field label="分类" value={article.category} onChange={value => setContent({ ...content, articles: content.articles.map((item, itemIndex) => itemIndex === index ? { ...item, category: value } : item) })} /><Field label="文章地址（英文或拼音）" value={article.slug} onChange={value => setContent({ ...content, articles: content.articles.map((item, itemIndex) => itemIndex === index ? { ...item, slug: value.toLowerCase().replace(/[^a-z0-9-]/g, "-").replace(/-+/g, "-").replace(/^-|-$/g, ""), href: `/notes/${value}` } : item) })} /><Field label="发布日期" value={article.publishedAt} onChange={value => setContent({ ...content, articles: content.articles.map((item, itemIndex) => itemIndex === index ? { ...item, publishedAt: value } : item) })} /><Field area label="摘要" value={article.excerpt} onChange={value => setContent({ ...content, articles: content.articles.map((item, itemIndex) => itemIndex === index ? { ...item, excerpt: value } : item) })} /><Field area label="正文（段落之间空一行）" value={article.body} onChange={value => setContent({ ...content, articles: content.articles.map((item, itemIndex) => itemIndex === index ? { ...item, body: value } : item) })} /><label className="studio-check"><input type="checkbox" checked={article.published} onChange={event => setContent({ ...content, articles: content.articles.map((item, itemIndex) => itemIndex === index ? { ...item, published: event.target.checked } : item) })} />公开发布</label><Link href={`/notes/${article.slug}`} target="_blank">预览文章 <ArrowUpRight /></Link></div>)}
          <button className="add-row" onClick={() => { const slug = `note-${Date.now()}`; setContent({ ...content, articles: [...content.articles, { id: Date.now(), category: "随笔", title: "新文章", excerpt: "文章摘要", publishedAt: new Date().toISOString().slice(0, 10), slug, body: "在这里写正文。", published: false, href: `/notes/${slug}` }] }); }}><Plus />新建草稿</button>
        </EditorSection>
