import { ArrowRight, ArrowUpRight, BookOpenCheck, Bot, BrainCircuit, Database, GitPullRequest, ShieldCheck } from "lucide-react";
...
export function WorkShowcase({ content }: { content: SiteContent }) {
  return <main className="work-page">
    <SiteHeader content={content} compact />
    <section className="work-hero" aria-labelledby="work-title"><div><p className="work-eyebrow"><DecryptedText text="SELECTED WORK / ENGINEERING" /></p><h1 id="work-title">我把 AI 用到了<br/><em><GradientText>系统层。</GradientText></em></h1><p className="work-hero__lead">不是只会调用模型。我让 Codex 与 Claude Code 共享外部记忆，把输入编译成可复用知识，部署长期运行的 Hermes Agent，并把业务数据流程做成可操作产品。</p><div className="work-hero__actions"><a className="work-button work-button--primary" href="#systems">按证据判断我的水平</a><a className="work-button" href="https://github.com/xmhuangzhijun-hue" target="_blank" rel="noreferrer">检查 GitHub <ArrowUpRight size={16}/></a></div></div><aside className="hero-index"><p>你会在这里看到</p><div><strong>01</strong><span>跨 Agent 外部记忆</span><small>Codex ↔ Claude Code</small></div><div><strong>02</strong><span>知识编译流水线</span><small>输入启发 → 可复用知识</small></div><div><strong>03</strong><span>生产 Agent 实例</span><small>Hermes 部署、运维与修复</small></div><div><strong>04</strong><span>业务产品与开源贡献</span><small>可登录 Demo + 5 个 PR</small></div></aside></section>
    <nav className="work-case-index" aria-label="案例索引">{systems.map(item => <a href={`#${item.id}`} key={item.id}>{item.label}</a>)}</nav>
    <section className="capability-line" data-reveal aria-label="AI 能力链"><p>INPUT</p><span>溯源与分析</span><p>MEMORY</p><span>跨端共享事实</span><p>AGENT</p><span>理解与执行</span><p>BUSINESS</p><span>真实产品验收</span></section>
    <section className="work-ledger" data-reveal id="systems"><div className="work-section-heading"><p>SYSTEMS, NOT PROMPTS</p><h2>五个系统，回答“具体做到了什么”。</h2><span>公开材料用于核验能力；私人数据、客户账户和生产凭据不进入展示。</span></div>{systems.map(({icon:Icon,...item})=><AnimatedContent key={item.id}><SpotlightCard className="system-case" id={item.id} role="article" aria-label={item.title}><header><span>{item.number}</span><Icon size={24}/><p>{item.label}</p><h3>{item.title}</h3><p className="system-intro">{item.intro}</p></header><details className="case-details"><summary>查看实现路径与公开证据 <ArrowRight size={16} /></summary><div className="system-flow">{item.flow.map((step,index)=><div key={step}><small>{String(index+1).padStart(2,"0")}</small><span>{step}</span>{index<item.flow.length-1&&<ArrowRight size={14}/>}</div>)}</div><div className="system-bottom"><div><p className="system-kicker">可检查的实现细节</p><ul>{item.facts.map(fact=><li key={fact}><ShieldCheck size={15}/>{fact}</li>)}</ul></div><div className="system-links"><p className="system-kicker">公开证据</p>{item.links.map(([label,href])=><a href={href} target={href.startsWith("/")?undefined:"_blank"} rel={href.startsWith("/")?undefined:"noreferrer"} key={href}>{label}<ArrowUpRight size={15}/></a>)}{"credential" in item&&<code><span>公开账号</span>{item.credential}</code>}</div></div></details></SpotlightCard></AnimatedContent>)}</section>
    <section className="work-method" data-reveal id="principles"><div><p className="work-eyebrow">ENGINEERING PRINCIPLES</p><h2>不把“AI 做了”当成完成。</h2><p>模型负责理解和判断，确定性系统负责事实与副作用，真实用户入口负责最终验收。</p></div><ol><li><span>01</span><strong>事实有来源</strong><p>当前代码与运行状态优先，历史用于避免重犯。</p></li><li><span>02</span><strong>记忆有治理</strong><p>候选不能覆盖 shared，吸收必须经验证。</p></li><li><span>03</span><strong>动作有回执</strong><p>queued、committed 和 verified 不能混为一谈。</p></li><li><span>04</span><strong>贡献可审查</strong><p>公开仓库、PR、测试和边界都允许外部质疑。</p></li></ol></section>
    <section className="work-contact" data-reveal><p>如果你在招聘 AI Agent、AI 应用工程或 FDE，欢迎直接检查我的工作。</p><a href="https://github.com/xmhuangzhijun-hue" target="_blank" rel="noreferrer">查看全部公开仓库 <ArrowUpRight size={18}/></a></section><SiteFooter content={content} />
  </main>;
}
